...
import { Check, Clock, ShieldCheck, Truck, Package, Flag } from 'lucide-react';
...
const DeliveryTimeline = ({ status, timestamps = {} }) => {
    ...
    <div className="py-4">
      <div className="flex flex-col gap-6">
        {STAGES.map((stage, index) => {
          ...
          const time = timestamps[`${stage.id}At`];

          return (
            <div key={stage.id} className="flex gap-4 relative">
              {/* Connector Line */}
              {index < STAGES.length - 1 && (
                <div className={`absolute left-3.5 top-8 w-[1px] h-10 transition-colors duration-500 ${
                  isCompleted ? 'bg-brand-green' : 'bg-white/5'
                }`} />
              )}

              {/* Icon / Indicator */}
              <div className={`w-7 h-7 rounded-full flex items-center justify-center z-10 transition-all duration-500 ${
                isCompleted ? 'bg-brand-green text-black' :
                isActive ? 'bg-brand-green text-black ring-4 ring-brand-green/20' :
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between gap-2">
                  <p className={`text-xs font-bold font-display uppercase tracking-wider transition-colors ${
                    isActive ? 'text-brand-green' : isCompleted ? 'text-gray-300' : 'text-gray-600'
                  }`}>
                    {stage.label}
                  </p>
                  {time && (
